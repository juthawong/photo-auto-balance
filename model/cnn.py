...
from opendeep.data.dataset import *
...
class CNN():
  ...
  def train(self,X,y,ratio_train,epoch):
    target_v  = T.dvector('y')
    loss      = MSE(
      inputs=self.net.models[-1].get_outputs(), # TAOTODO: Dense object
      targets=y)

    error_monitor = Monitor(
      name='error', 
      expression=mean(neq(self.net.models[-1].get_outputs(), y)), 
